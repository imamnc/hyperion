...
class KinoService extends BaseService implements ServiceContract
{
    ...
    public function blacklist(array $request)
    {
        ...
        // Change flag_status from string to boolean
    ...
    public function pengadaanList(array $request)
    {
        ...
        try {
            ...
            if ($request['metode_pemilihan_penyedia_id'] == 5) { // If Ekspedisi
                $raw = Http::withoutVerifying()->post($this->project->url . '/paket/selectPaketEkspedisi', $request);
                // Get response mentah
                $raw = json_decode($raw->body(), true);
                $data = $raw['result']['data']['getData'];
            } else {
                $raw = Http::withoutVerifying()->post($this->project->url . '/paket/admin/select', $request);
                // Get response mentah
                $raw = json_decode($raw->body(), true);
            ...
            foreach ($data as $key => $dat) {
                ...
                // Validate tipe pengadaan
                if ($detail == null) {
                    throw new \Exception("Paket ID tidak ditemukan !", 422);
                }
                // Request tahapan
