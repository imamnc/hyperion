...
class MandiriService extends BaseService implements ServiceContract
{
    ...
    public function login(array $request)
    {
        ...
        try {
            // Request login to KINO
            $raw = Http::withoutVerifying()->post($this->project->url . '/auth/login/admin', $form)->json();
            $login_data = $raw['data'];
            ...
            // Get user
            $user = User::query()->where('email', '=', $request['email'])->where('project_id', '=', $project->id)->first();
            ...
            // Get user detail data from eproc endpoint
            $raw = Http::withoutVerifying()->withToken($user->service_token)->post($this->project->url . '/auth/get_session', ['session_id' => $user->service_token])->json();
            // Format response data
            $eproc_user = $raw['data']['session_data'];
            // Update company_id & pegawai_id
            $user->fill([
                'company_id' => $eproc_user['company_id'],
                'pegawai_id' => $eproc_user['pegawai_id']
            ]);
            $user->save();
            // Return token
    ...
    public function vendorList(array $request)
    {
        ...
        $request['pegawai_id'] = auth()->user()->pegawai_id;
        $request['id_bu'] = "";
        $request['vag_id'] = "";
        $request['rekanan_id'] = "";
        $request['notglfilter'] = true;
        $request['status'] = 0;
        $request['otv'] = false;
    ...
    public function pengadaanList(array $request)
    {
        ...
        try {
            ...
            foreach ($data['data'] as $key => $dat) {
                ...
                $pr = (count($raw_pr['result']['data']) > 0) ? $raw_pr['result']['data'][0] : null;
                // Generate response
                $response[] = [
    ...
    public function pengadaanDetail(array $request)
    {
        ...
        try {
            ...
            $response['tahapan'] = $response_tahapan;
            $response['pekerjaan'] = $pekerjaans;
    ...
    public function PRList(array $request)
    {
        $request = [
            "selectBy" => 'namaPr',
            "selectByValue" => isset($request['keyword']) ? $request['keyword'] : '',
            "status" => 1,
            "cari" => null,
            "offset" => ($request['page'] - 1) * $request['limit'],
        ...
        try {
            ...
            foreach ($data as $key => $dat) {
                $response[] = [
                    ...
                    'kode_barang' => (isset($dat['material_number'])) ? $dat['material_number'] : null,
    ...
    public function PRDetail(array $request)
    {
        try {
            ...
            $response[] = [
                ...
                'kode_barang' => (isset($detail['material_number'])) ? $detail['material_number'] : null,
    ...
    public function contractList(array $request)
    {
        ...
        try {
            ...
            $data = $raw['result']['data'];
            // Apply pagination using collection
            $data = collect($data);
            $data = $data->slice($request['offset'], $request['limit'])->toArray();
            // Format response
            $response = [];
            foreach ($data as $key => $dat) {
                $response[] = [
                    ...
                    'nomor_pr' => (isset($dat['pr_kode'])) ? $dat['pr_kode'] : null,
    ...
    {
        $request = ['id_manajemen_kontrak' => $request['id']];

        try {
            // Try to request
            $raw = Http::withoutVerifying()->post($this->project->url . '/manajemen-kontrak/getDataKontrakById', $request)->json();
            $data = $raw['result']['data'];
            ...
            $response = [
                ...
                'nomor_pr' => (isset($data['pr_kode'])) ? $data['pr_kode'] : null,
